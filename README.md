...
全程在一念的设置面板里完成，不需要命令行。

1. 设置 → 插件 → 安装本插件 → 点「启用」，面板会分三步引导：
   - **第 2 步有一个「打开 Apple 账号页」按钮**，点了会用系统浏览器打开
     [account.apple.com](https://account.apple.com)，照提示进「登录与安全」→
     「App 专用密码」生成一个。它是 16 位、形如 `abcd-efgh-ijkl-mnop`，**只显示一次**。
     前提是账号已开**双重认证**，没开的话页面上不会有这个入口。
   - 填 Apple 账号与那 16 位密码，点「**测试连接**」——通过会列出你的日历。
2. 新建一个同步实例，选要同步哪些日历（下拉直接从 iCloud 列）、拉取的时间窗口。
3. 要用排期镜像的话，打开「启用排期镜像」，然后点「**测试镜像写入**」。

### 为什么写入要单独测一次

**读得通不代表写得进去。** `MKCALENDAR` 在部分 Apple 账号上会被拒（企业管理的账号、
家庭共享里的受限成员），而这决定了镜像能不能自己建日历。「测试镜像写入」会走一遍
完整路径——建日历、写一条测试事件、再删掉——让你在**打开开关之前**就知道结果，
而不是等第一次同步之后去插件日志里找一条 403。

被拒的话解决办法很简单：先在 Mac 或 iPhone 的日历 App 里手工建一个 iCloud 日历，
再把「镜像日历名称」改成它的名字。

这个 action 的时间预算只有 15 秒（SDK 的 `TIMEOUTS.custom`），而完整路径要 6 次
CalDAV 往返。所以它按剩余预算走（`src/budget.mts`），iCloud 慢的时候会**提前返回
已经验证到的部分结论**并让你再点一次——被宿主杀掉的话用户只看到「插件超时」，
什么信息都没有。

> **App 专用密码是长期有效的完整日历读写凭据**，而一念一期的插件是没有沙箱的普通用户态进程（契约 §9）。填进来等于把 iCloud 日历的读写权限交给本插件进程。不接受这一点就不要启用它。
>
> 另外：改过 Apple 账号主密码后，所有 App 专用密码会被自动吊销，届时同步会全部报 401，需要重新生成一个填回来。
...
单测覆盖不了「iCloud 到底允不允许」。**用户不需要跑这个**——面板上的「测试连接」
与「测试镜像写入」做的是同一件事。这个脚本的价值在于**不装一念也能验**，
改 CalDAV 层时比反复打包安装快得多：

```bash
npm run build
APPLE_ID=you@icloud.com APPLE_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx node scripts/probe.mjs
# 加 --write 才会验证 MKCALENDAR / PUT / DELETE（会建一个临时日历）
node scripts/probe.mjs --write
```

它比面板多报一项：**`sync-collection` 支持度**。那个只影响同步快慢（不支持就每轮
走全量窗口，能用但慢），不值得占用户一个按钮。
